class RPSMove
	...
	def <=>(other_move)
		RPSMoveRules.rule_comparison_result(type, other_move.type)
	end
...
class RPSMoveRules
	...
	MOVE_STR_SELECTION = STR_MOVECLASS_MAP.keys.freeze
	MOVE_CLASS_SELECTION = STR_MOVECLASS_MAP.values.freeze
	MOVE_NAME_LENGTHS = MOVE_CLASS_SELECTION.uniq.map { |move_cls| move_cls.new.to_s.length }
	...
	def self.rule_comparison_result(move_type_a, move_type_b)
		...
		if rules.key?(move_type_a) && rules[move_type_a].key?(move_type_b)
			...
			err_msg = "No rule found for move comparison between #{move_type_a} and #{move_type_b}"
