...
def tfidf(term, document, documents)
  f = tf(term, document) * idf(term, documents)
  # round to first decimal to check tests given
  f.round(1)
end
...
def idf(term, documents)
  ...
  case number_of_documents_with_term
  when 0
    0
  else
...
=begin
  The problem is one of integer division instead of  floating point
  division, which is why some fractional numbers that have not been
  truncated are showing and the rest show's zero.
  The tf method always returns an Integer that represents the 
  term frequency, the idf method passes an expression that is an 
  Integer division to the Math.log method, which returns a float.
  Allthough IRB does not reflect this, the Integer division causes
  the wrong quotient to be passed to the log method in the first place,
  which not the correct result.

  Casting the division operands before the division to Float fixes the
  problem and gives the expected result.

  Furthermore, the final frequency in the tfidf method is then rounded
  to the first decimal place in order to check the test cases given,
  this may not be desired in the actual usage of the method though.

  Further exploration:
  
  The problem that occurs when document1 is removed from documents is
  that the division can be a division by zero, which we have to account
  for before actually dividing by the denominator. In order to do this
  whe check if the denominator is zero and if it is the result is 0,
  since both division by zero and log of zero are undefined, and compute
  the quotient with valid denominator otherwise.
